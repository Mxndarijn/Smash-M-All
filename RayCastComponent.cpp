...
RayCastComponent::RayCastComponent(glm::vec2 webcamResolution, std::list<glm::vec2>* points) : webcamResolution(webcamResolution),
opticalCenter(glm::vec2(webcamResolution.x / 2, webcamResolution.y / 2)), points(points)
{
...
void RayCastComponent::draw()
{
	...
	for (auto& line : lines) {
		glVertex3f(line.x, line.y, line.z - 100);
		glVertex3f(line.x, line.y, line.z + 100);
	}
	glEnd();
...
glm::vec3 RayCastComponent::pointToVec3(glm::vec2 point)
{
	...
	if (cameraComponent) {
		projection_matrix = cameraComponent->getMatrix();
	}
	...
	position.x = transformed_coords[0] - gameObject->position.x;
	position.y = -transformed_coords[1] - gameObject->position.y;
	position.z = 0;
